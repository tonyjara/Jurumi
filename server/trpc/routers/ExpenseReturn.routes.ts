  ...
  edit: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        await caller.expenseReturn.cancelById({ id: input.id });
        await caller.expenseReturn.create(input);
  ...
  cancelById: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      return await prisma.$transaction(async (txCtx) => {
        const expenseReturn = await txCtx.expenseReturn.update({
          where: { id: input.id },
          data: { wasCancelled: true },
          include: { transactions: true },
        });
        await cancelTransactionsAndRevertBalance({
          transactions: expenseReturn.transactions,
          txCtx,
        });
